...
static int mlv_snd_m6ii_start_audio_now(void)
{
    ...
    if(msgs < 2)
    {
        trace_write(trace_ctx, "M6II SoundDev: fewer than two audio buffers");
        return 0;
    }
    ...
    if (rc)
    {
        ...
        mlv_snd_current_buffer = NULL;
        mlv_snd_next_buffer = NULL;
        trace_write(trace_ctx, "M6II SoundDev start failed rc=%d", rc);
        return 0;
    }
...
static void mlv_snd_start()
{
    ...
    if (is_camera("M6II", "1.1.1") && mlv_snd_m6ii_start_sem)
    {
        ...
        {
            mlv_snd_m6ii_worker_waiting = 0;
            trace_write(trace_ctx, "M6II SoundDev start worker creation failed");
        }
    }
}
...
static void mlv_snd_cbr_started(uint32_t event, void *ctx, mlv_hdr_t *hdr)
{
    ...
    if (is_camera("M6II", "1.1.1"))
    {
        ...
        {
            trace_write(trace_ctx, "M6II SoundDev start worker unavailable");
            return;
        }
